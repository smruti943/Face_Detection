...
namespace Emotion.Model
{
    ...
    public class EmotionDetectionAsset : BaseAsset
    {
        ...
        /// </summary>
        public Dictionary<Int32, List<DetectedEmotions>> EmotionsHistory = new Dictionary<Int32, List<DetectedEmotions>>();
        ...
        public Boolean DetectEmotionsInLandmarks()
        {
            ...
            foreach (DetectedFace kvp in Faces)
            {
                ...
                //
                if (EmotionsHistory.Count > 0)
                {
                    EmotionsHistory = new Dictionary<Int32, List<DetectedEmotions>>();
                }
        ...
        public Boolean DetectEmotionsInLandmarks(List<dynamic> imagePoints)
        {
            Int32 ndx = 0;

            foreach (var kvp in imagePoints)
            {
                var expandoDict = kvp as IDictionary<string, object>;
                List<object> value = expandoDict.Values.ToList();
                DetectedEmotions DetectedEmotions = new DetectedEmotions();
                ...
                foreach (POINT p in Vectors)
                {
                    var expandoX = imagePoints[p.X] as IDictionary<string, object>;
                    List<object> listX = expandoX.Values.ToList();

                    var expandoY = imagePoints[p.Y] as IDictionary<string, object>;
                    List<object> listY = expandoY.Values.ToList();

                    EuclideanDistances.Add(EuclideanDistance(Convert.ToInt32(listX[0]), Convert.ToInt32(listX[1]), Convert.ToInt32(listY[0]), Convert.ToInt32(listY[1])));
                }
        ...
        public Boolean ProcessFaces(List<dynamic> imagePoints)
        {
            DetectLandmarksInFaces(imagePoints);
...
#warning This code must be more versatile to support more formats
        ...
        public Boolean ProcessLandmarks(List<dynamic> imagePoints)
        {
            return DetectEmotionsInLandmarks(imagePoints);
        }
        ...
        private void DetectLandmarksInFaces(List<dynamic> imagePoints)
        {
            foreach (KeyValuePair<RECT, List<POINT>> kvp in Faces)
            {
                int markcount = 0;
